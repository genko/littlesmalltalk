...
static int binaryPrims(int number, object firstarg, object secondarg)
{
    ...
    case 7: /* symbol set */
        nameTableInsert(symbols, strHash(charPtr(firstarg)), firstarg,
                        secondarg);
        break;
...
static int strUnary(int number, char *firstargument)
{
    ...
    case 7: /* value of symbol */
        returnedObject = nameTableLookup(symbols,firstargument);
        break;
...
object primitive(register int primitiveNumber, object *arguments)
{
    ...
    else
    {
        ...
        case 1:
            ...
            break;
        case 2:
            returnedObject = binaryPrims(primitiveNumber - 20, arguments[0],
                                         arguments[1]);
            break;
        case 3:
            returnedObject = trinaryPrims(primitiveNumber - 30, arguments[0],
        ...
        case 7: /* integer binary operations */
            if ((!isInteger(arguments[0])) || !isInteger(arguments[1]))
                ...
                returnedObject = intBinary(primitiveNumber - 60,
                                           intValue(arguments[0]), intValue(arguments[1]));
            break;

        case 8: /* string unary */
            returnedObject = strUnary(primitiveNumber - 80,
                                      charPtr(arguments[0]));
            break;

        case 10: /* float unary */
            returnedObject = floatUnary(primitiveNumber - 100, floatValue(
                                            arguments[0]));
            break;

        case 11: /* float binary */
            returnedObject = floatBinary(primitiveNumber - 110, floatValue(
                                             arguments[0]), floatValue(arguments[1]));
            break;
