    ...
    while (--timeSliceCounter > 0)
    {
        ...
        if (debugging)
        {
            fprintf(stderr, "method %s %d ",
                    charPtr(basicAt(method, messageInMethod)), byteOffset);
        ...
        case PushConstant:
            ...
            case contextConst:
                ...
                if (contextObject == processStack)
                {
                    ...
                    returnPoint = intValue(processStackAt(linkPointer+2));
                    contextObject = newContext(linkPointer, method, copyFrom(
                                                   processStack, returnPoint, linkPointer
                                                   - returnPoint), copyFrom(processStack,
                                                           linkPointer + 5, methodTempSize(method)));
...
doFindMessage:
            ...
            else
            {
                ...
                if (!findMethod(&methodClass))
                {
                    ...
                    ipush(messageToSend);
                    messageToSend = newSymbol(
                                        "message:notRecognizedWithArguments:");
                    ipush(argarray);
        ...
        case DoPrimitive:
            ...
            case 87: /* value of symbol */
                returnedObject = nameTableLookup(symbols,charPtr(*primargs));
                break;
            default:
...
doReturn:
            returnPoint
            = intValue(basicAt(processStack, linkPointer + 2));
